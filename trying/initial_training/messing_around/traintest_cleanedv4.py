...
#%%
class TomogramDataGenerator(Sequence):
    """
    Keras data generator for loading and processing tomogram data and generating heatmaps on-the-fly.
    """

    def __init__(self, tomograms, particle_locations_df, particle_types, batch_size=2, shuffle=True):
        """
        Initialization.
        ...
        self.particle_locations_df = particle_locations_df
        self.particle_types = particle_types
        self.batch_size = batch_size
        self.shuffle = shuffle
        self.indices = np.arange(len(self.tomograms))
        self.on_epoch_end()
    ...
    def __len__(self):
        ...
        """
        return int(np.ceil(len(self.tomograms) / self.batch_size))
    ...
    def __getitem__(self, index):
        ...
        end_idx = min((index + 1) * self.batch_size, len(self.tomograms))
        ...
        for idx in batch_indices:
            tomo = self.tomograms[idx]
            run_name = tomo['run']
            tomogram_data = tomo['tomogram']

            # Normalize tomogram
            tomogram_normalized = (tomogram_data - tomogram_data.min()) / (tomogram_data.max() - tomogram_data.min())
            tomogram_normalized = tomogram_normalized.astype(np.float32)
            tomogram_normalized = tomogram_normalized[..., np.newaxis]  # Add channel dimension
            X_batch.append(tomogram_normalized)

            # Filter labels for the current run
            labels = self.particle_locations_df[self.particle_locations_df['run'] == run_name]

            # Generate heatmaps for this tomogram
            if not labels.empty:
                heatmaps = generate_heatmaps_optimized_max(labels, tomogram_data.shape, self.particle_types)
            else:
                # If no labels, initialize empty heatmaps
                heatmaps = np.zeros((tomogram_data.shape[0], tomogram_data.shape[1],
                                     tomogram_data.shape[2], len(self.particle_types)), dtype=np.float32)
...
# For example, extract a cube from the first tomogram
# Adjust the indices based on your tomogram's dimensions and label distribution
...
# Extract the regions
trainer_tom = tomograms_train[0]['tomogram'][train_z_start:train_z_end, train_y_start:train_y_end, train_x_start:train_x_end]
...
if not trainer_labels.empty:
    ...
    trainer_heatmaps = np.zeros((trainer_tom.shape[0], trainer_tom.shape[1], trainer_tom.shape[2], len(particle_types)), dtype=np.float32)
...
if not validator_labels.empty:
    ...
    validator_heatmaps = np.zeros((validator_tom.shape[0], validator_tom.shape[1], validator_tom.shape[2], len(particle_types)), dtype=np.float32)
